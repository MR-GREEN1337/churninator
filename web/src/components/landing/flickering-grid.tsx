...
export const FlickeringGrid: React.FC<FlickeringGridProps> = ({
  ...
  const memoizedColor = useMemo(() => {
    return getRGBA(themeAwareColor);
  }, [themeAwareColor]);
    ...
    (
      ...
      for (let i = 0; i < cols; i++) {
        for (let j = 0; j < rows; j++) {
          ...
          // --- START FIX ---
          // Replaced .some() with a manual for loop to avoid a potential
          // build tool/fast refresh bug that mis-transpiles the arrow function.
          if (maskCtx && maskCanvas) {
            ...
            for (let k = 0; k < maskData.length; k += 4) {
              // Check the alpha channel (k+3) to see if a pixel is visible.
              if (maskData[k + 3] > 0) {
                hasText = true;
                break; // Exit early once we find a pixel from the text mask.
              }
            }
          }
          // --- END FIX ---
